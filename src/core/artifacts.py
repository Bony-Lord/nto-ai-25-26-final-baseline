"""Artifacts manager with step status and atomic writes."""
...
class ArtifactsManager:
    """Manage cache status for pipeline steps."""
    ...
    def should_run(self, step_name: str, fingerprint: str, output_path: Path) -> bool:
        """Return True when step must run."""
    ...
    def _mark_done_internal(
        ...
        """Persist successful step completion with optional duration."""
        status = self._read_json(self.step_status_path)
        status.setdefault(step_name, {})
    ...
    def mark_done(
        ...
        """Persist successful step completion."""
    ...
    def get_step_durations(self, step_names: list[str]) -> dict[str, float]:
        """Read historical step durations from status metadata."""
        status = self._read_json(self.step_status_path)
...
def atomic_write_json(path: Path, payload: dict[str, Any]) -> None:
    """Write JSON atomically via temporary file in same directory."""
...
def atomic_write_dataframe(df: pd.DataFrame, path: Path) -> None:
    """Write DataFrame atomically to csv or parquet by file extension."""
    path.parent.mkdir(parents=True, exist_ok=True)
