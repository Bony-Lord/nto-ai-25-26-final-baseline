...
- В импортах используется namespace `src.competition.solution.*`, хотя физически файлы лежат прямо в `src/competition/`.
- В текущем baseline config key `user_genre` не совпадает с `source`, который выставляет `user_genre.py` (`user_genre_popularity`). Это важно, если вы будете настраивать веса или разбирать blend по источникам.
