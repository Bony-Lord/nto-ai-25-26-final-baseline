"""User genre profile to edition candidates."""
...
class UserGenrePopularityGenerator:
    """Generate candidates from user genre preferences and edition genre links."""
    ...
    def generate(
        ...
        catalog_genres = dataset.catalog_df[["edition_id", "book_id"]].merge(
            dataset.book_genres_df[["book_id", "genre_id"]], on="book_id", how="inner"
        )
        ...
        for user_id, group in tqdm(
            ...
            top_items = sorted(
                score_by_edition.items(), key=lambda x: (-x[1], x[0])
            )[:k]
